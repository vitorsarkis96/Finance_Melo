...
input[type=text],input[type=number],select{
  background:var(--s2);border:1px solid var(--border2);color:var(--ink);
  padding:9px 12px;border-radius:8px;font-size:13px;width:100%;outline:none;transition:border .15s;font-weight:400
}
...
.btn-ghost{background:var(--s2);color:var(--ink);border:1px solid var(--border2)}
...
.sync-dot{width:6px;height:6px;border-radius:50%;background:var(--lime)}
...
/* ─── RESPONSIVE ─── */
@media(max-width:900px){.g4{grid-template-columns:1fr 1fr}.g5{grid-template-columns:1fr 1fr}.g3{grid-template-columns:1fr 1fr}}
@media(max-width:600px){.g4,.g3,.g2,.g5{grid-template-columns:1fr}.tabs{flex-wrap:wrap}}
`;

/* ─── HELPERS ─── */
const R = (v, p = "R$") => isNaN(v) || v === null ? "—" : `${p} ${Number(v).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
const P = (v) => (!isFinite(v) || isNaN(v)) ? "—" : `${v.toFixed(1)}%`;
let uid = 200;
const id = () => ++uid;
...
function useStore() {
  const [d, setD] = useState(() => {
    try {
      const saved = localStorage.getItem("melo_health");
      return saved ? JSON.parse(saved) : INIT;
    } catch { return INIT; }
  });
  const set = useCallback(fn => setD(p => {
    const n = fn(p);
...
export default function App() {
  ...
  /* computed */
  const totalRec = d.receitas.reduce((a, r) => a + +r.val, 0);
  const mrr = d.receitas.filter(r => r.tipo === "recorrente").reduce((a, r) => a + +r.val, 0);
  const arr = mrr * 12;

  const catTotals = {};
  COST_CATS.forEach(c => { catTotals[c.key] = (d.custos[c.key] || []).reduce((a, x) => a + +x.val, 0); });
  const totalCustos = Object.values(catTotals).reduce((a, v) => a + v, 0);
  ...
  const churned = d.clientes.filter(c => !c.ativo).length;
  const churnRate = d.clientes.length > 0 ? (churned / d.clientes.length) * 100 : 0;
  const avgMRR = ativos.length > 0 ? ativos.reduce((a, c) => a + +c.mrr, 0) / ativos.length : 0;
  ...
  /* mutations */
  const addRec = o => set(p => ({ ...p, receitas: [...p.receitas, { ...o, id: id() }] }));
  const delRec = i => set(p => ({ ...p, receitas: p.receitas.filter(x => x.id !== i) }));
  const updRec = (i, f, v) => set(p => ({ ...p, receitas: p.receitas.map(x => x.id === i ? { ...x, [f]: v } : x) }));

  const addCost = (cat, o) => set(p => ({ ...p, custos: { ...p.custos, [cat]: [...(p.custos[cat] || []), { ...o, id: id() }] } }));
  const delCost = (cat, i) => set(p => ({ ...p, custos: { ...p.custos, [cat]: p.custos[cat].filter(x => x.id !== i) } }));
  const updCost = (cat, i, f, v) => set(p => ({ ...p, custos: { ...p.custos, [cat]: p.custos[cat].map(x => x.id === i ? { ...x, [f]: v } : x) } }));

  const addCli = o => set(p => ({ ...p, clientes: [...p.clientes, { ...o, id: id() }] }));
  const delCli = i => set(p => ({ ...p, clientes: p.clientes.filter(x => x.id !== i) }));
  const updCli = (i, f, v) => set(p => ({ ...p, clientes: p.clientes.map(x => x.id === i ? { ...x, [f]: v } : x) }));

  const nwSet = (k, f, v) => setNw(p => ({ ...p, [k]: { ...p[k], [f]: v } }));
  const nwGet = k => nw[k] || {};
  const nwClr = k => setNw(p => ({ ...p, [k]: {} }));

  // ── ASAAS SYNC ─────────────────────────────────────────────────
  const [syncing, setSyncing] = useState(false);
  const [syncInfo, setSyncInfo] = useState<{at: string; total: number} | null>(() => {
  ...
  const syncAsaas = useCallback(async () => {
    ...
    try {
      ...
      // Mescla receitas do Asaas com as manuais (mantém manuais, substitui as do Asaas)
      set((p: any) => {
    ...
    <>
      ...
      <div className="wrap">
        ...
        <div className="topbar">
          <div>
            <div style={{ fontFamily: "var(--body)", fontSize: 22, fontWeight: 700, letterSpacing: 3, color: "var(--ink)", lineHeight: 1 }}>MELO <span style={{ color: "var(--lime)" }}>HEALTH</span></div>
            <div style={{ fontSize: 10, color: "var(--ink2)", marginTop: 4, letterSpacing: ".5px", fontWeight: 400 }}>Saúde financeira em tempo real</div>
          </div>
        ...
        <div className="sync-bar">
          <div>
            ...
            {!syncError && syncInfo && (
              <span className="sync-info" style={{marginLeft:8}}>
                Última sync: <strong>{new Date(syncInfo.at).toLocaleString("pt-BR")}</strong>
                {" · "}Recebido no mês: <strong style={{color:"var(--lime)"}}>R$ {Number(syncInfo.total).toLocaleString("pt-BR",{minimumFractionDigits:2})}</strong>
              </span>
            )}
...
/* ─── KPI CARD ─── */
function KPI({ label, value, sub, color = "var(--ink)", dot }) {
  return (
    <div className="card-sm">
      <div className="kpi-label">{label}</div>
      <div className="kpi-val" style={{ color }}>{value}</div>
      {sub && <div className="kpi-sub">{dot && <span className="kpi-dot" style={{ background: color }} />}{sub}</div>}
    </div>
  );
}

/* ─── DASHBOARD ─── */
function TabDash({ d, mrr, arr, totalRec, totalCustos, catTotals, lucro, mBruta, mLiq, ativos, churnRate, avgMRR, ltv, health, hColor, hLabel }) {
  return (
    <div>
      {/* Row 1 */}
      <div className="g4">
      ...
      <div className="g2">
          ...
          <div className="ring-wrap">
            <div className="ring" style={{ background: `conic-gradient(${hColor} ${health}%, var(--s3) 0)`, boxShadow: `0 0 24px ${hColor}33` }}>
              <div className="ring-inner">
                <div style={{ fontFamily: "var(--body)", fontSize: 28, fontWeight: 700, color: hColor, lineHeight: 1 }}>{health}</div>
                <div style={{ fontSize: 9, color: "var(--ink2)", letterSpacing: 1 }}>/ 100</div>
              </div>
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: "var(--body)", fontSize: 20, fontWeight: 700, color: hColor, letterSpacing: .5, marginBottom: 14 }}>{hLabel}</div>
              {[
        ...
        <div className="card">
            ...
            {COST_CATS.filter(c => catTotals[c.key] > 0).sort((a,b) => catTotals[b.key]-catTotals[a.key]).map(c => (
              <div key={c.key} className="bar-row">
                ...
                <div className="bar-track">
                  <div className="bar-fill" style={{ width: `${totalCustos > 0 ? (catTotals[c.key] / totalCustos) * 100 : 0}%`, background: c.color }} />
                </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <div>
              <div className="kpi-label">Total Custos</div>
              <div style={{ fontFamily: "var(--body)", fontSize: 22, fontWeight: 700, color: "var(--red)" }}>{R(totalCustos)}</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div className="kpi-label">Resultado</div>
              <div style={{ fontFamily: "var(--body)", fontSize: 22, fontWeight: 700, color: lucro >= 0 ? "var(--lime)" : "var(--red)" }}>{R(lucro)}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {ativos.length > 0 && (
        <div className="card">
          ...
          <table className="tbl">
            ...
            <tbody>
              {ativos.sort((a,b) => b.mrr - a.mrr).map(c => {
...
/* ─── RECEITAS ─── */
function TabRec({ d, mrr, totalRec, addRec, delRec, updRec, nwSet, nwGet, nwClr }) {
  const n = nwGet("rec");
  return (
    <div>
      <div className="g3">
            ...
            <tbody>
              {d.receitas.map(r => (
                <tr key={r.id}>
                  ...
                  <td>
                    <select value={r.tipo} onChange={e => updRec(r.id, "tipo", e.target.value)} style={{ width: "auto" }}>
        ...
        <div className="add-row">
          ...
          <div className="add-grid">
            <div style={{ flex: 3, minWidth: 160 }}>
              ...
              <input type="text" value={n.desc || ""} placeholder="Ex: Retainer cliente" onChange={e => nwSet("rec", "desc", e.target.value)} />
            </div>
            ...
            <div style={{ flex: 1, minWidth: 100 }}>
              ...
              <input type="number" value={n.val || ""} placeholder="0,00" onChange={e => nwSet("rec", "val", +e.target.value || 0)} />
            </div>
            <button className="btn btn-lime" style={{ alignSelf: "flex-end" }} onClick={() => {
              if (n.desc) { addRec({ desc: n.desc, tipo: n.tipo || "recorrente", val: n.val || 0 }); nwClr("rec"); }
            }}>Adicionar</button>
          </div>
        </div>
      </div>
...
/* ─── CUSTOS ─── */
function TabCost({ d, catTotals, totalCustos, addCost, delCost, updCost, nwSet, nwGet, nwClr }) {
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "var(--ink2)", marginTop: 1 }}>{cat.desc}</div>
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontFamily: "var(--body)", fontSize: 20, fontWeight: 700, color: cat.color }}>{R(total)}</div>
                  <div style={{ fontSize: 10, color: "var(--ink2)", letterSpacing: 1 }}>{items.length} item{items.length !== 1 ? "s" : ""}</div>
                </div>
                ...
                <div className="add-row" style={{ marginTop: 0 }}>
                  ...
                  <div className="add-grid">
                    <div style={{ flex: 3, minWidth: 140 }}>
                      <div className="field-label">Descrição</div>
                      <input type="text" value={n.desc || ""} placeholder={cat.desc.split(",")[0]} onChange={e => nwSet(`cost_${cat.key}`, "desc", e.target.value)} />
                    </div>
                    <div style={{ flex: 1, minWidth: 100 }}>
                      <div className="field-label">Valor (R$)</div>
                      <input type="number" value={n.val || ""} placeholder="0,00" onChange={e => nwSet(`cost_${cat.key}`, "val", +e.target.value || 0)} />
                    </div>
                    <button className="btn btn-lime" style={{ alignSelf: "flex-end" }} onClick={() => {
                      if (n.desc) { addCost(cat.key, { desc: n.desc, val: n.val || 0 }); nwClr(`cost_${cat.key}`); }
                    }}>Adicionar</button>
                  </div>
                </div>
              </div>
...
/* ─── CLIENTES ─── */
function TabCli({ d, ltv, avgMRR, ativos, churnRate, totalRec, addCli, delCli, updCli, nwSet, nwGet, nwClr }) {
  const n = nwGet("cli");
  const cr = churnRate > 0 ? churnRate / 100 : 1 / 24;
  return (
    <div>
      ...
      <div className="card">
          ...
          <tbody>
            {d.clientes.map(c => (
              <tr key={c.id}>
                ...
                <td>
                  <select value={c.ativo ? "ativo" : "churned"} onChange={e => updCli(c.id, "ativo", e.target.value === "ativo")} style={{ width: "auto" }}>
        ...
        <div className="add-row">
          ...
          <div className="add-grid">
            {[{ k: "nome", l: "Nome", t: "text", f: 3, ph: "Nome do cliente" }, { k: "mrr", l: "MRR (R$)", t: "number", f: 2, ph: "0" }, { k: "meses", l: "Meses Ativo", t: "number", f: 1, ph: "0" }].map(f => (
              <div key={f.k} style={{ flex: f.f, minWidth: 80 }}>
                <div className="field-label">{f.l}</div>
                <input type={f.t} value={n[f.k] || ""} placeholder={f.ph}
                  onChange={e => nwSet("cli", f.k, f.t === "number" ? +e.target.value || 0 : e.target.value)} />
              </div>
            ))}
            <button className="btn btn-lime" style={{ alignSelf: "flex-end" }} onClick={() => {
              if (n.nome) { addCli({ nome: n.nome, mrr: n.mrr || 0, meses: n.meses || 0, ativo: true }); nwClr("cli"); }
            }}>Adicionar</button>
          </div>
        </div>
      </div>
...
/* ─── ALERTAS ─── */
function TabAlerts({ lucro, mLiq, churnRate, mrr, totalRec, ativos, totalCustos, catTotals }) {
  const alerts = [];
  if (lucro < 0) alerts.push({ t: "red",  m: `Operação no prejuízo de ${R(Math.abs(lucro))}. Custos superam receitas em ${P(Math.abs(mLiq))}.` });
  if (mLiq < 15 && mLiq >= 0) alerts.push({ t: "amber", m: `Margem líquida em ${P(mLiq)} — abaixo dos 20% recomendados para agências de serviço.` });
  if (churnRate > 10) alerts.push({ t: "red",   m: `Churn crítico: ${P(churnRate)}. A cada cliente perdido, você perde ${R(mrr / ativos.length || 0)}/mês em média.` });
  else if (churnRate > 5) alerts.push({ t: "amber", m: `Churn moderado: ${P(churnRate)}. Monitore NPS e entrega de resultados.` });
  if (mrr < totalRec * 0.5 && totalRec > 0) alerts.push({ t: "amber", m: `Apenas ${P((mrr / totalRec) * 100)} da receita é recorrente. Priorize contratos de retainer.` });
  if (ativos.length < 4) alerts.push({ t: "amber", m: `${ativos.length} clientes ativos — concentração de risco alta. Um churn impacta ${P(ativos.length > 0 ? 100 / ativos.length : 0)} da receita.` });
  ...
  if (alerts.length === 0) alerts.push({ t: "lime", m: "Todos os indicadores dentro dos parâmetros saudáveis. Siga monitorando." });

  const recs = [
    { icon: "→", text: `MRR atual ${R(mrr)} — próxima meta: ${R(mrr * 1.2)} (+20%)` },
    { icon: "→", text: `Mínimo ideal: 6 clientes ativos para diluir risco de concentração` },
    { icon: "→", text: `Margem líquida alvo para agências: 25–35%` },
    { icon: "→", text: `Churn saudável: < 3% ao mês. Onboarding e entrega são a chave` },
    { icon: "→", text: `LTV:CAC alvo de 3:1. Calcule seu CAC por canal para validar` },
    { icon: "→", text: `Ferramentas e programas: manter abaixo de 5% da receita` },
  ];

  const cls = { red: "al-red", amber: "al-amber", lime: "al-lime", blue: "al-blue" };
      ...
      <div className="g2">
        <div>
          <div className="card" style={{ marginBottom: 14 }}>
            ...
            {alerts.map((a, i) => (
              <div key={i} className={`alert ${cls[a.t]}`}>{a.m}</div>
            ))}
          </div>
        </div>
        <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {recs.map((r, i) => (
            <div key={i} style={{ display: "flex", gap: 10, padding: "10px 14px", background: "var(--s2)", borderRadius: 8, border: "1px solid var(--border)" }}>
              <span style={{ color: "var(--lime)", fontWeight: 700, flexShrink: 0 }}>{r.icon}</span>
              <span style={{ fontSize: 12, color: "var(--ink2)", lineHeight: 1.5, fontWeight: 400 }}>{r.text}</span>
            </div>
          ))}
        </div>
      </div>
